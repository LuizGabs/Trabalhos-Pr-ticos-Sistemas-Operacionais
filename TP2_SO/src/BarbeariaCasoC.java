...
public class BarbeariaCasoC {
	    public static void main(String[] args) {
	                ...
	                if (ninguemNaFila < 3) {
	                    try {
	                        ...
	                        if (categoria == 1 && somaVetor(cadeirasOcupadasPorCategoria) < numCadeiras) {
	                            ...
	                            //System.out.println("Sargento Tainha trouxe uma pausa.");
	                    ...
	                    //System.out.println("Sem ninguém na fila 3 vezes segindas! Sargento Tainha já pode ir embora!");
	            ...
	            while (cont < 3) {
	                ...
	                if (filaEsperaOficiais.isEmpty()) {
	                	...
	                	try {
	                        ...
	                        cadeirasOcupadasPorCategoria[1]--;
	                        cont = 0;
		                	numAtendimentosPorCategoria[1]++;
		                    tempoTotalAtendimentoPorCategoria[1] += TempoDeCorte;
		                    // Tempo de atendimento simulado
		                    Thread.sleep(rand.nextInt(TempoDeCorte));
	                	} catch (InterruptedException e) {
		                    e.printStackTrace();
	        ...
	        Thread barbeiroDentinho = new Thread(() -> {
	        	int cont = 0;
	            while (cont < 3) {
	                Random rand = new Random();
	                int TempoDeCorte = 0;
	                if (filaEsperaSargentos.isEmpty()) {
		                try {
		                    // Chamada para atender um cliente
		                    int categoria = atenderCliente(mutex, filaEsperaOficiais, filaEsperaSargentos, filaEsperaCabos, filaEsperaPausas);
		                  //  System.out.println(categoria);
		                    ...
		                    // Impressão da categoria do cliente atendido pelo Recruta Zero
		                 //   System.out.println("Barbeiro Dentinho atendeu um cliente: " + getCategoriaString(categoria));
		                    // Tempo de atendimento simulado
		                    Thread.sleep(rand.nextInt(TempoDeCorte));
		                } catch (InterruptedException e) {
		                    e.printStackTrace();
	                	...
	                	try {
	                        ...
	                        cadeirasOcupadasPorCategoria[2]--;
	                        cont = 0;
		                	numAtendimentosPorCategoria[2]++;
		                    tempoTotalAtendimentoPorCategoria[2] += TempoDeCorte;
	                        Thread.sleep(rand.nextInt(TempoDeCorte));
	                	} catch (InterruptedException e) {
		                    e.printStackTrace();
		                }
	        ...
	        Thread barbeiroOtto = new Thread(() -> {
	        	int cont = 0;
	            while (cont < 3) {
	                Random rand = new Random();
	                int TempoDeCorte = 0;
	                if (filaEsperaCabos.isEmpty()) {
		                try {
		                    // Chamada para atender um cliente
		                    int categoria = atenderCliente(mutex, filaEsperaOficiais, filaEsperaSargentos, filaEsperaCabos, filaEsperaPausas);
		                  //  System.out.println(categoria);
	                	...
	                	try {
	                        ...
	                        cadeirasOcupadasPorCategoria[3]--;
	                        cont = 0;
		                	numAtendimentosPorCategoria[3]++;
		                    tempoTotalAtendimentoPorCategoria[3] += TempoDeCorte;
	                        Thread.sleep(rand.nextInt(TempoDeCorte));
	                	} catch (InterruptedException e) {
		                    e.printStackTrace();
		                }
	        ...
	        Thread tenenteThread = new Thread(() -> {
	         ...
	         // Variáveis para rastrear o comprimento médio das filas
	            ...
	            while (dia < 1000) { // Executa a cada 1000 interações do Sargento Tainha
	                ...
	                System.out.println("Relatório do Tenente Escovinha - Dia " + dia);
	                ...
	                if (numAtendimentosPorCategoria[0]!=0 && numAtendimentosPorCategoria[1]!=0 && numAtendimentosPorCategoria[2]!=0 && numAtendimentosPorCategoria[3]!=0) {
		                System.out.println("Tempo médio de atendimento por categoria: (em segundos)");
		                ...
		                System.out.println("Tempo médio de espera por categoria: (em segundos)");
